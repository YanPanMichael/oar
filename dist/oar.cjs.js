...
// 合并默认配置
Object.assign(Axios__default["default"].defaults, defaultRequestConfig);
/**
 * 对 Oar 注入拦截器
 */
function setInterceptors(oar) {
    ...
    oar.interceptors.request.use(function (request) {
        ...
        if (request.method === 'post' &&
            ...
            request.data = qs__namespace.stringify(request.data);
        }
        return request;
    });
    /**
...
// 新建 Axios 实例
var Oar = Axios__default["default"].create();
// 全局 Oar 设置拦截器
setInterceptors(Oar);
// 在调用 Oar.create() 创建实例后，也会对其注入默认拦截器
Oar.create = function (config) {
    if (config === void 0) { config = {}; }
    var axiosInstance = Axios__default["default"].create(config);
    // oar 实例设置拦截器
    return setInterceptors(axiosInstance);
};var win = isClient() ? window : undefined;
var oar = win && win.oar ? win.oar : Oar;module.exports=oar;