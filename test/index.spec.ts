...
describe('@ferry-core/oar 基础功能测试.', () => {
  it('应该可以创建一个 oar 实例.', () => {
    ...
    expect(Object.getPrototypeOf($oarInstance).constuctor).toEqual(
      Object.getPrototypeOf($axiosInstance).constuctor,
    );
  });
...
describe('@ferry-core/oar CSRF防御功能测试', () => {
  ...
  it('xsrfCookieName 默认应该为csrf_cookie', async () => {
    const res = await oar.post('postData');
    expect(cookie.getItem('csrf_cookie')).toBe(
      qs.parse(res.config.data).csrf_token,
    );
  });
  ...
  it('post 方法在 data 中已经设置了不为空的 csrf_token 时，不再对 data 中重新注入 csrf_token', async () => {
    ...
    await oar
      .post('/postData', { csrf_token: 'validcsrftoken' })
      .then((res) => {
  ...
  it('post 请求允许修改 csrf 字段名', async () => {
    await oar
      .post('/postData', {}, { csrfKeyName: 'csrf_token_custom' })
      .then((res) => {
        expect(qs.parse(res.config.data).csrf_token_custom).not.toBe(undefined);
      });

    await oar
      .post('/postData', {}, { csrfKeyName: ['csrf_token1', 'csrf_token2'] })
      .then((res) => {
